...
class TableViewController: UITableViewController {
	...
	func letsConnect(completion: @escaping(Receipts) -> ()) {
		var str = "products="
		...
		let jsonUrlString = "http://127.0.0.1:8000/receipt/?" + str
		print(jsonUrlString)
		guard let url = URL(string: jsonUrlString) else { return completion([]) }
		var request = URLRequest(url: url)
		request.addValue("token \(Credentials.instance.token)", forHTTPHeaderField: "Authorization")
		URLSession.shared.dataTask(with: request){ data, response, err in
			guard let data = data else {return}
			//print(dataAsString!)
			do{
				let courses = try JSONDecoder().decode(Receipts.self, from: data)
				self.parse = courses
				print(courses)
				completion(courses)
			} catch {
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		...
		letsConnect { data in
			DispatchQueue.main.async {
				...
				let destinationTVC: CollectionViewController = segue.destination as! CollectionViewController
				destinationTVC.abc = data
				destinationTVC.collectionView?.reloadData()
			}
		}
	}
}
